document.getElementById("content").querySelectorAll(TocField).forEach(function(heading, i) { // runs a function for all headings inside your rich text element
  ...
  (TocField).split(",").forEach(function(x) { // runs a function for each item in your headings list
  ...
  // document.querySelectorAll('.toc-multiple').forEach(tocMultiple => tocMultiple.appendChild(item));
});
